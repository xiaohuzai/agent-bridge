...
npm releases: GitHub Actions → **Publish npm** (manual `workflow_dispatch`, takes a semver input) — stamps the version, runs the full suite as a gate, publishes `@xiaohuzai/agent-bridge` with the repo secret `NPM_TOKEN` (granular token, read-write on the @xiaohuzai scope, bypass-2FA), then tags the release commit. main's `package.json` version is aligned opportunistically in the next regular PR (the ruleset blocks direct pushes). Two release-day traps (paid 2026-09-12): the workflow's own PR step fails (`GitHub Actions is not permitted to create or approve pull requests`) — open `release/<ver>` → main manually; and commits pushed with `GITHUB_TOKEN` don't trigger CI, so the release PR gets no `Test` check until an empty commit is pushed from the user token. The first-run hint in cli.mjs resolves `agents.example.json` relative to the CLI itself, so it works identically from a repo checkout and an npm install.
...
- **JSON-RPC ids may be strings**: approval/permission answers must echo the agent's RAW rpc id — coercing through `Number()` answers `id:null` and the agent blocks forever on the unanswered request. Also: an admitted turn has NOTHING pending tracking it (v2 `session/prompt` is a mere ack), so a dead child must sweep in-flight turns with a `… mid-turn` error or the client's stream hangs forever; pending rpcs and turn entries carry the child identity so a late `exit` event can never settle a newer child's work. And v1 `tool` events carry the tool's own id in an additive `id` field (the `done.stopReason` pattern): clients pair one call's started/completed on it — the detail strings differ (" (exit N)" suffixes), and pairing on name|detail splits one call into two tool rows.
...
- Tests drive the REAL adapter and REAL HTTP server against scripted fake agents (`test/fake-codex-app-server.mjs`, `test/fake-acp-agent.mjs`) — executable shebang scripts speaking JSONL on stdio, with prompt markers (APPROVE/SLOW/FAIL/IMG/MIXED/EDIT/TOOLS/DIE, STRID modifier) triggering the interesting paths. No codex/claude install, no network.
...
1. Live verification of both adapters against real agents — codex: done for the wire (native adapter AND the official codex-acp shim). claude: real turns verified on the user's Mac 2026-09-08 (streaming, session continuity, usage, approvals) via the official claude-agent-acp; handshake/version-negotiation/error-path verified in the container; disconnect-interrupt and bridge-restart resume test-covered but not exercised live. pi: registry-verified 2026-09-11 via svkozak/pi-acp 0.0.33 + pi 0.85.1 driven by a local mock OpenAI provider (real turns incl. tool calls, restart resume) — this surfaced the `session/resume` → `session/load` fallback now in acp-stdio.mjs (facts in its header). gemini not planned yet.
...
4. Submit to the ACP Registry — SUBMITTED 2026-09-12 (upstream PR: agentclientprotocol/registry#598, listing `@xiaohuzai/agent-bridge@0.3.0` with the zero-setup `agent-bridge acp claude` spawn; runbook + entry record: `docs/acp-registry.zh-CN.md`): the registry lists agents only, and the stdio front is what makes the bridge itself one.
